...
extern "C" {
    pub(super) fn __switch(
        current_task_ctx_ptr: *mut SwitchCtx,
        next_task_ctx_ptr: *const SwitchCtx,
    );
}
...
lazy_static::lazy_static! {
    pub static ref PROCESS_MANAGER: ProcessManager = unsafe {
        ProcessManager::new()
    };
}
...
impl ProcessManager {
    unsafe fn new() -> Self {
        ...
        let inner = UPSafeCell::new(ProcessManagerInner { next: 0, load });
        trace!("all task loaded");
        Self { num, inner }
    }
    fn get_next_process(&self) -> Option<Arc<ProcessControlBlock>> {
        let mut inner = self.inner.get_mut();
        if inner.next == inner.load.len() {
            None
        } else {
            inner.next += 1;
            Some(inner.load[inner.next - 1].clone())
        }
    }
    pub fn get_current_process(&self) -> &ProcessControlBlock {
        let inner = self.inner.get();
        &inner.load[inner.next - 1]
    }
}
...
fn trap_from_user() -> ! {
    ...
    let ctx = PROCESS_MANAGER.get_current_process().trap_ctx();
...
fn kernel_fail(inst_addr: usize, hint: &str) -> ! {
    error!("[kernel]{}", hint);
    println!("instrument at {:#x}", inst_addr);

    run_next_process();
}

pub fn run_next_process() -> ! {
    match PROCESS_MANAGER.get_next_process() {
        Some(_) => restore_to_user(),
        None => shutdown(false),
    }
}
pub fn restore_to_user() -> ! {
    set_user_trap_entry();
    let satp = PROCESS_MANAGER.get_current_process().satp();
